...
| `--emit-code --url <url>` | Print ONE setup code, built from what you pass on this line, and nothing else on stdout. Reads no saved setup, writes nothing, sends no request — so it works on a build rig before the gateway is exposed. Same payload builder as `--setup`, so the code is byte-identical to the one the wizard would print for the same gateway. Key from `CONDUCK_TOKEN`; `--keyless` declares a gateway that has none, and an *unset* variable is refused rather than read as keyless. Options: `--model <id>`, `--name <shown-in-app>` (custom gateways), `--kind openclaw\|hermes\|custom`, `--transport tailscale\|funnel\|cloudflare\|public`, `--files-url <url>` with `CONDUCK_FILES_PASS`. The code carries your gateway key |
...
The check changes no host configuration. It sends a handful of real requests—which may consume compute or enter server-side history—and grades the answers against the rules at **[conduck.com/setup/adapter/v1/](https://conduck.com/setup/adapter/v1/)**. That includes the one the pairing wizard cannot prove: your token check is actually **enforced** (a missing or wrong token must get `401` on both routes). Exit code `0` means every check passed. Plain `http://` is accepted toward an address only your own network can reach — a loopback or private-IP literal, an IPv6 ULA or link-local literal, or a `.local` name; a public address or a DNS name gets refused without a request being sent (same rule as the wizard's pairing prompt, below). The token comes from `$CONDUCK_TOKEN` or a hidden prompt, never the command line (`argv` is visible to `ps`). For a scripted run with no keys at all, export `CONDUCK_TOKEN=` **empty** — that is an explicit keyless declaration. Leaving it unset where no prompt is possible (piped or closed input) fails fast instead of grading the target as keyless, because a silently-assumed keyless run grades the wrong thing and calls it a pass. Add `--deep` to test a message with an image (an honest HTTP `400` "images unsupported" answer passes).

One of its checks reads what your adapter *said* rather than how it was shaped. `CHAT_FRAMING` asks whether the reply to `Reply with exactly: pong` contains `pong` — as a substring, compared case-blind, so `Pong.` and `PONG` pass. Every other chat check grades the envelope and none of them looks at the words, so an adapter that wraps each turn in a persona, a system prompt or an agent scaffold heavy enough to lose the user's instruction answers something fluent and unrelated and passes them all. If this one is red, the framing your adapter puts around a turn is reaching the engine and the person's own words are not.

Two details decide borderline replies. First, the prompt is **subtracted before the search**: the instruction contains the very word it asks for, so every case-blind copy of `Reply with exactly: pong` is cut out of the reply and only what remains is searched. An adapter that hands back the request, or a rendered transcript quoting it, therefore scores nothing — those are the shapes a plain substring test used to pass. Second, a reply that is **exactly the prompt and nothing else**, trimmed, passes on purpose. That is the development echo engine from step 5 of the build brief answering as designed, and the ordinary profile is meant to reach `exit 0` against it; the carve-out is exact, so the prompt buried inside a longer reply stays red. Treat a green `CHAT_FRAMING` as a floor rather than a certificate: it proves one requested word came back outside your own framing, not that the framing leaves a real instruction intact — for that, run a real engine behind the adapter and read a real answer.
...
The last line of every noninteractive adapter check is a machine summary (`CONDUCK_CHECK_ADAPTER schema=4 …`) — build scripts key on the prefix, the `schema=`, and the exit code. The grammar is frozen per schema number; any change to it bumps `schema=`, so pin the number you parse. Its `profile=` field names both axes independently — `basic`, `deep`, `basic+files`, `deep+files` — so a `--files` run is never mistaken for a plain one.

Both checks are legitimately slow: every real turn waits up to five minutes, so the worst case is roughly 20 minutes for `--check-server`, 25 for `--check-adapter`, 30 with `--deep` and 35 with `--files`. A turn in flight prints a heartbeat to **stderr** every 30 seconds, which is what tells a redirected run apart from a wedged one. Neither the heartbeat nor anything else it prints reaches stdout, so `tail -1` still finds the summary.
...
`--setup` mints one at the end of a verified run and `--show-code` re-emits a saved one. `--emit-code` is the third minter and the only one a script can drive: it builds the same payload from what you hand it, with no terminal, no saved profile and no network. Reach for it rather than assembling the base64 yourself.
