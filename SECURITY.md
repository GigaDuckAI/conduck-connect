...
It gets asked often enough to answer here. Where the address is encrypted, the wizard requires your gateway to present a certificate your devices **already trust**, and refuses a self-signed one, a private CA, an expired one or a wrong hostname. It does not offer to pin a fingerprint instead. That is a standing decision, not an omission:
...
A plain `http://` address is accepted when it is one only the local network can reach — a loopback or private IP literal, an IPv6 ULA or link-local literal, or a `.local` name. A **single-label name is refused, rooted or not** (`nas`, `ollama`, `uz`, `uz.`): real one-label TLDs answer at the public DNS root, so a resolver falling through its search domains could carry the bearer token in cleartext to a public host — the same machine's `.local` name or its IP literal is the working spelling. That is not a hole in the section above: an unencrypted address presents no certificate, so there is no chain to evaluate, nothing a pin could tighten, and no route by which it becomes a way to use a self-signed one. The app refuses a saved fingerprint paired with such an address rather than honouring the address and silently dropping the pin.

The boundary is not a preference either side invented. Apple's App Transport Security decides it from the address string before any connection is attempted, and it permits exactly the shapes above: a **domain name is refused however private the machine behind it is**, and so is the carrier-grade NAT range (`100.64.0.0/10`) an overlay VPN hands out. Accepting more here would mint a setup code that scans and then fails, so the wizard predicts that verdict instead of discovering it later on a phone. Where it cannot reproduce the platform's own address grammar in shell it refuses rather than guesses, which makes it slightly stricter than the app and never looser. Where a shape was never measured against ATS at all, both sides refuse it unless the kernel confines the traffic regardless — `0.0.0.0/8`, `::` and deprecated site-local `fec0::/10` are refused on that ground, while `127.0.0.0/8` and `fe80::/10` stay accepted because a packet to either cannot leave the local link whatever the platform would have said. A wrong refusal costs one message; a wrong allowance costs the token.

What it costs is stated plainly at every prompt that accepts one, and again on the screen that prints the code: nothing on that lane is encrypted, so anyone on the network can read the messages **and the bearer token that rides every request** — and the address only works while the device is on that same network, which rules out a standalone Apple Watch. Plain `http://` is taken when you type it and is never suggested, defaulted to, or produced by prepending a scheme to a bare host.
