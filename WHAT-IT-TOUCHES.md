...
| `~/.hermes/.env` | Hermes: discover `API_SERVER_PORT` (validated as a whole 1–65535 port; anything else is reported and the default used) and read `API_SERVER_KEY`. | **Yes, with your yes** — the `API_SERVER_*` keys appended, and the file's mode tightened to `0600` if other accounts could read it. Row below. |
...
| Enable Hermes API server | Appends a blank line, a dated `# added by conduck-connect` comment, and `API_SERVER_ENABLED` / `API_SERVER_HOST` / `API_SERVER_PORT` to `~/.hermes/.env`, then restarts `hermes-gateway`. An `API_SERVER_KEY` already present is reused, never rotated; a fourth `API_SERVER_KEY` line is appended only when none exists. If the file has to be created, it is created `0600` (the key lands inside it). **Permissions:** an existing `.env` that other accounts on the host can read is tightened to `0600` right after the append — its *contents* change only as you approved above, and the tightening is reported on screen either way. | Remove the appended block (from the `# added by conduck-connect` comment down) and restart. The `0600` mode stays until you change it back. |
