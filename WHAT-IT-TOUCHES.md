...
| `~/.hermes/config.yaml` | Hermes file lane: check the API-server file toolset, `terminal.cwd`, terminal backend, and explicit global file-tool disables. Ambiguous/custom mappings fail closed. | **Yes, with your yes** — only the narrow file-lane keys described below. |
...
| Align Hermes file-lane configuration | With your yes, sets `terminal.cwd` to the exact shared folder and, only when an explicit `platform_toolsets.api_server` list has narrowed away file access, adds the `file` bundle without removing any other bundle. Non-local backends, global file-tool disables, symlinks, and ambiguous YAML are reported for manual resolution rather than widened. Hermes is restarted and the narrow result re-read before proceeding. | Restore the previous values shown before confirmation and restart Hermes. |
| Agent-guidance block in `.hermes.md` / `HERMES.md` (file lane only, Hermes) | Appends (or refreshes in place) the same marker-delimited Conduck rules in the Hermes project context that will actually load: exact uploaded paths, completed root-level output writes, and plain reply filenames. Everything outside the markers is untouched; symlinked or malformed files are refused. | Delete the block between (and including) the two markers. |
...
- **Restarting Hermes** where there is no `hermes-gateway` systemd user unit — the restart command is printed for you to run.
...
- Normal setup and `--show-code` verification may send a local health check,
  ...
  an HTTPS PUT→GET→DELETE probe. An OpenClaw or Hermes lane gets one additional
  real agent turn: a randomized input is uploaded, the selected agent must read
  it with its file tools, write byte-identical output at the shared root before
  replying, and name that output. The connector removes and proves the exact
...
`bash`, `curl`, `python3` (always required); `openssl` (setup and `--show-code` — it mints credentials and computes certificate pins; the two check commands never reach it and do not require it); `tailscale`, `cloudflared`, `rclone` (only for the path you pick). A missing prerequisite → the script names every one it needs, prints the install command for your OS, and exits with status `1` before changing anything; re-run to resume.
